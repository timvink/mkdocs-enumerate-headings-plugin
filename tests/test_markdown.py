...
def test_enumerate_headings():
    ...
    headings = {1: "# test", 2: "## test", 3: "# test", 4: "## test", 5: "## test"}
    assert markdown.enumerate_headings(headings) == {
        1: "# 1. test",
        2: "## 1.1 test",
        3: "# 2. test",
    ...
    headings = {2: "## sub", 3: "# head"}
    assert markdown.enumerate_headings(headings) == {2: "## 0.1 sub", 3: "# 1. head"}
...
def test_heading_depth():
    ...
    assert [markdown.heading_depth(x) for x in heading_lines] == [1, 2, 2, 3, 3, 2]
...
def test_update_heading_chapter():
    ...
    assert (
        markdown.update_heading_chapter(line, 24) == "#### 24.4.2.3 Another chapter!!1!"
    )
